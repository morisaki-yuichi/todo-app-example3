...
## スプリント10: E2E + 教材最終化（最終スプリント）

- 計画: [スプリント10 バックログ](../10_sprint10/backlog.md) /
  記録: [レビュー](../10_sprint10/review.md)・[レトロスペクティブ](../10_sprint10/retrospective.md)
- PR: [#10 スプリント10: E2E + 教材最終化](https://github.com/morisaki-yuichi/todo-app-example3/pull/10)
- このスプリントの概念: [E2E テストとテストピラミッド](concepts.md#e2e-テストとテストピラミッド)

### Step 10-1: Playwright E2E

- 差分: [GitHub](https://github.com/morisaki-yuichi/todo-app-example3/commit/dd85aba) /
  ローカル: `git show dd85aba`

**何を・なぜ**: 本物のブラウザ + 本物の API + 本物の DB で動く「最後の砦」を
2本だけ書きます（Try T-18: 単体テストで守れているものは重複させない）。

1. **クリティカルパス**: 登録 → 作成 → 完了 → 削除（全層の結合が正しい証明）
2. **認可**: 他人の TODO が一覧に出ない・URL 直叩きでも読めない
   （US-08 フロント編の受け入れ条件。S4 実験⑤の「漏えい」をユーザー視点で再演）

**足場の作り方**

| ファイル | 作り方 |
|---|---|
| `frontend/package.json` ほか | CLI で更新: `npm install -D @playwright/test` → `npx playwright install chromium` |
| `frontend/playwright.config.ts` | 手で新規作成 |
| `frontend/e2e/todo-flow.spec.ts` | 手で新規作成 |
| `frontend/vite.config.ts` | 既存ファイルを手で編集（Vitest の include を src/ に限定。**e2e を Vitest が拾うと jsdom で Playwright API を実行しようとして落ちる**） |
| `frontend/.gitignore` | 既存ファイルを手で編集（test-results / playwright-report） |

**実行方法**:

```bash
# 前提: ルートで docker compose up -d + マイグレーション適用済み
cd frontend
npm run e2e      # dev サーバは Playwright が自動起動（起動済みならそれを使う）
```

読みどころ:

- **一意なメールアドレス生成**（タイムスタンプ + 乱数）: DB を洗い替えずに
  何度でも実行できる。「写経者の環境で値が変わっても壊れない」テストの書き方
- `page.once('dialog', d => d.accept())`: window.confirm の受諾。
  クリックの**前に**仕掛ける（後だと間に合わない）
- セレクタは `getByRole` / `getByLabel`: 画面の文言・ロールで探す =
  ユーザーが見ているものと同じ手がかりでテストする

**わざと失敗を見る実験⑫: バックエンドを止めて E2E を実行**

`docker compose stop api` してから実行すると、「登録」の URL 遷移待ちで
**タイムアウト**します。単体テストと違い、失敗メッセージから直接原因は読めません。
Playwright が自動保存する **test-results/…/error-context.md（失敗時の画面
スナップショット）** を開くと「登録に失敗しました」のアラートが写っており、
そこから API の死を辿れます。E2E は「環境全体が前提」であること、
失敗調査はスナップショット・トレースから入ること、が学びです。
→ [S10 レビュー記録](../10_sprint10/review.md#実験)

**よくあるエラーと症状**

| 症状 | 原因の辿り方 |
|---|---|
| 全テストがタイムアウト | バックエンド停止（実験⑫）か DB 未マイグレーション。まず `curl :8002/health` |
| `browserType.launch` の失敗 | `npx playwright install chromium` 忘れ。Linux で共有ライブラリ不足なら `--with-deps` |
| Vitest 実行時に e2e が落ちる | Vitest の include 設定漏れ（e2e は Playwright 管轄） |
| confirm で止まる | dialog ハンドラをクリック前に仕掛けているか確認 |

**ここでコミット**: `test: Playwright E2E を追加（クリティカルパスと認可の2シナリオ）`

**CI に入れない判断**: E2E の CI 組み込みは4層（DB + API + フロント + ブラウザ）の
構築とフレーク対策が必要で、教材のスコープ外とした（バックログの棚卸し参照）。
発展課題として演習編に掲載。

### Step 10-2: 教材の最終整備

キャッチアップ集（[catch-up.md](catch-up.md)）と演習編（[exercises.md](exercises.md)）を
追加し、ロードマップ・バックログ・README を最終状態に更新（各1コミット）。

---

# 完走後の地図（このリポジトリの歩き方・最終版)

写経を終えた人・これから始める人向けの全体像:

```text
入口   catch-up.md   … 他フレームワークとの対応表（クローン前でも読める）
本編   dev-walkthrough.md（本ガイド）… Step 1-1 〜 10-1 をコミット単位で追体験
辞書   concepts.md   … 出てきた概念を「定義・なぜ・実例」で引く
記録   docs/0N_sprintN/ … 各スプリントの計画・レビュー・レトロ（トラブルの生ログ）
出口   exercises.md  … ドリルと機能追加演習、発展課題
```

**対比学習の推奨ポイント**（このリポジトリならではの読み方）:

- **cookie セッション vs JWT**: PR #4 と PR #8 の差分を並べて読む
- **素の fetch vs TanStack Query**: PR #6〜7 と PR #9 の差分を並べて読む
- **失敗の記録**: 各スプリントレビューの「実験」「トラブル記録」だけを
  通しで読むと、このスタックの落とし穴カタログになる（12実験 + トラブル多数）
...
| PBI-18 E2E + 教材最終化（US-08 フロント編の E2E 含む） | S10 の各コミット | [#10](https://github.com/morisaki-yuichi/todo-app-example3/pull/10) |
...
| 実験⑫: バックエンド停止で E2E → タイムアウト（error-context.md から辿る） | [S10 レビュー記録](../10_sprint10/review.md#実験) |
