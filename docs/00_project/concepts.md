...
### サーバ状態とキャッシュ（TanStack Query）

**① 一言定義**: サーバ状態 = 「本当の持ち主がサーバにいるデータ」（TODO 一覧など）。
TanStack Query はそのキャッシュ・再取得・状態管理を専門にするライブラリ。

**② なぜ必要か**: サーバ状態はフォーム入力のようなクライアント状態と性質が違う——
古くなる・他人が変える・取得に時間がかかる。素の fetch + useEffect で扱うと、
3状態の手書き・StrictMode 対応・再取得の仕組みを毎画面書くことになる
（S6〜S7 で体験済み）。さらに「画面に戻るたび毎回ローディング」という UX の限界は、
手書きでは越えにくい（キャッシュを自作することになる）。
TanStack Query は「まずキャッシュを見せて、裏で取り直す」
（stale-while-revalidate）を既定の動きとして提供する。

**③ 実例**: [frontend/src/main.tsx](../../frontend/src/main.tsx)（QueryClient）と
各ページ。**何が消えたか**の一覧は
[Step 9 の対応表](dev-walkthrough.md#移行で消えたものこのスプリントの主旨)。
api 層（S6 T-10）がそのまま queryFn / mutationFn になり、画面の書き換えだけで
移行が完了した。

### queryKey と無効化

**① 一言定義**: queryKey はキャッシュの「住所」（例: `['todos', {page: 2}]`）。
invalidateQueries は「この住所（とその配下）のデータはもう古い」という宣言で、
表示中の画面は自動で再取得される。

**② なぜ必要か**: キャッシュを持つことの代償は「いつ古くなるかを誰かが知らせる」
責任。更新系（作成・変更・削除）の後に無効化を忘れると、**エラーも警告もなく
「更新したのに画面が変わらない」**バグになる（実験⑪で実演。しかも
refetchOnWindowFocus が「たまに直す」ため、再現条件が不可解に見える）。
S6 実験⑦の「依存配列の書き漏らし」と同型の問題が、より宣言的な形で残っている
と理解するとよい。

**③ 実例**: 一覧 `['todos', listParams]`・詳細 `['todo', id]` の設計と、
mutation の `invalidateQueries` / `setQueryData`（応答でキャッシュ直接更新）/
`removeQueries`（消えたものは削除）の3つの使い分け
→ [Step 9-2](dev-walkthrough.md#step-9-2-詳細画面の移行)
...
**① 一言定義**: E2E（End-to-End）テストは、本物のブラウザで本物のシステム全体
（フロント + API + DB）を操作する自動テスト。テストピラミッドは
「単体テスト多数・結合テスト適量・E2E 少数」という配分の指針。

**② なぜ必要か**: 単体テストはモックの境界の中でしか正しさを言えない
（本教材のフロントテストは api 層をモックしている = 「API と本当につながるか」は
検証していない）。E2E はその隙間を埋める唯一の証明。一方で E2E は遅く・
環境全体が前提で・失敗原因が遠い（実験⑫: バックエンド停止でただのタイムアウトに
見える）。だから**数を絞る**——単体で守れるものを E2E で重複させない。
本リポジトリの配分: バックエンド単体 63 + フロント単体 25 + E2E 2。

**③ 実例**: [frontend/e2e/todo-flow.spec.ts](../../frontend/e2e/todo-flow.spec.ts)
（クリティカルパス + 認可の2本のみ）。認可の E2E は S4 実験⑤の「漏えい」を
ユーザー視点で再演し、US-08 の受け入れ条件を機械化したもの
→ [Step 10-1](dev-walkthrough.md#step-10-1-playwright-e2e)

---

（概念解説集はここまで。全スプリントの概念を登場順に収録した）
